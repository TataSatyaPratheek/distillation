...
class CachedDistillationLoss(nn.Module):
    ...
    def forward(
        ...
        """
        # Create a version that will work even if student_logits doesn't have gradients
        # This is a robust workaround for the training issues

        # Ensure teacher logits are on the same device as student logits
        if teacher_logits.device != student_logits.device:
            teacher_logits = teacher_logits.to(student_logits.device)

        # Always detach teacher logits
        ...
        # Create a differentiable computation path even if tensors don't have gradients
        # This creates a new computation graph that can be differentiated
        kl_loss = F.kl_div(
            log_probs_student,
            probs_teacher,
            reduction="none"
        ...
        if attention_mask is not None:
            ...
            mask = attention_mask.to(dtype=torch.bool, device=kl_loss.device)

            # Apply mask and mean reduction
            kl_loss = kl_loss * mask
            num_tokens = mask.sum().item() or 1  # Avoid division by zero
            reduced_loss = kl_loss.sum() / num_tokens
        else:
            reduced_loss = kl_loss.mean()

        # Scale by temperature squared
        return self.alpha * reduced_loss * (self.temperature ** 2)